    ...
    fetch("http://127.0.0.1:5000/get-products/").then((request) => {
      request.json().then((obj) => {
        ...
        old_container.innerHTML = `<div class="item">
             ...
             <p class="product-price">T-shirt: ${product[2]}</p>
             <p class="product-date">Price: ${product[3]}</p> 
         </div>`;
      });
    });
  }
...
function createCards() {
  ...
  fetch("https://still-ocean-71718.herokuapp.com/view/").then((request) => {
    request.json().then((obj) => {
      console.log(obj);
      data = obj.data;
      console.log(data);
      ...
      data.forEach((product) => {
        container.innerHTML += `<div class="item">
           ...
           <p class="product-date">Name: ${product[2]}</p>
           <p class="product-price">Price: ${product[3]}</p>
           <button onclick="addToCart(${product[0]})" class="show-cart">add cart</button>
